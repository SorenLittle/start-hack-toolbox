...
def croptimization(number_of_fields: int, number_of_seasons: int, crops: list[Crop], initial_nutrients: int = 10, min_nutrients: int = 3, max_nutrients: int = 20 ):
    # Fields
    ...
    Covercrop = Crop(name=CropName.COVERCROP,crop_yield=0,duration=1, nutrient_impact=3)
    # Crops
    crops.append(Covercrop)
    C = [crop.name for crop in crops]

    C_without_CoverCrop = [crop.name for crop in crops if crop.name != CropName.COVERCROP.name]
    ...
    for f in F:
        ...
        # Constraint: No more than two of the same crop in each timestep
    for c in C_without_CoverCrop:
        for t in T:
            mdl.add_constraint(mdl.sum(x[c, t, f] for f in F) <= 2, f"max_two_same_crop_{c}_{t}")
